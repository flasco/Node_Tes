...
function tes(){
    for(let i = 1;i<=9;i++){
        q.push("http://www.23us.com/class/"+i+"_1.html");
    }
}
...
function fetchList(urlx,callback){
    HttpReq.crawlPage(urlx,(res)=>{
        ...
        let $2,asn;
        //   console.time("测试代码速度");
        for(let i = 1,size=ass.length ;i<size ; i++){
            $2 = cheerio.load(ass[i] ,{decodeEntities: false})
            asn = $2('td');
            novelList[i-1] = new Object();
            novelList[i-1].name = asn[0].children[1].children[0].data;
            novelList[i-1].latestChapter = asn[1].children[0].children[0].data;
            novelList[i-1].author = asn[2].children[0].data;
            console.log(i+' '+novelList[i-1].name);
        }
